...
const statusChipColor = (value: string) =>
  value === "Accepted"
    ? "success"
    : value === "Rejected"
      ? "error"
...
watch(
  [selectedJobTitle, selectedApplicationStatus],
  (
    [newSelectedJobTitle, newSelectedApplicationStatus],
    [oldSelectedJobTitle, oldSelectedApplicationStatus]
  ) => {
...
<template>
  <div>
    ...
    <div v-if="userRole === 'admin'">
      ...
      >
        ...
        <template #item.resume="{ item }">
          <div>
            <a
              :href="`http://127.0.0.1:8000/storage/${item.raw.resume}`"
              target="_blank"
        ...
        <template #item.status="{ item }">
          <VChip :color="statusChipColor(item.raw.status)">{{
            item.raw.status
          }}</VChip>
        </template>
        ...
        <template #item.actions="{ item }">
          <div class="d-flex gap-1">
            <IconBtn>
              <VIcon
                icon="tabler-edit"
                @click="handleApplicantEdit(item.raw.id)"
              />
            </IconBtn>
            <IconBtn>
              <VIcon
                icon="tabler-trash"
                @click="handleApplicantDelete(item.raw.id)"
              />
            </IconBtn>
          </div>
        </template>
    ...
    <EditApplicantsDrawer
      :is-drawer-open="editApplicantDrawer"
      @close-dialog="dialogClose"
      :application-edit-id="applicationEditId"
    />
    <!-- dialog for deleting applicant's status  -->
    <DeleteJobApplicationDialogBasic
      :delete-id="applicationDeleteId"
      :is-dialog-visible="deleteApplicantDialog"
