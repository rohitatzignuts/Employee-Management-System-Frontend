...
var arrow = (options) => ({
  ...
  async fn(state) {
    ...
    const offset = within(min2, center, max2);
    const shouldAddOffset = getAlignment(placement) != null && center != offset && rects.reference[length] / 2 - (center < min2 ? paddingObject[minProp] : paddingObject[maxProp]) - arrowDimensions[length] / 2 < 0;
  ...
  return {
    ...
    async fn(state) {
      ...
      const {
        ...
        flipAlignment = true
      } = options, detectOverflowOptions = _objectWithoutPropertiesLoose(options, _excluded2);
      const side = getSide(placement);
...
var shift = function shift2(options) {
  ...
  return {
    ...
    async fn(state) {
      ...
      } = options, detectOverflowOptions = _objectWithoutPropertiesLoose(options, _excluded4);
      const coords = {
        x,
        y
      };
...
function getInnerBoundingClientRect(element2, strategy) {
  ...
  const scale = isHTMLElement(element2) ? getScale(element2) : {
    x: 1,
    y: 1
  };
...
function getRectRelativeToOffsetParent(element2, offsetParent, strategy) {
  const isOffsetParentAnElement = isHTMLElement(offsetParent);
  const documentElement = getDocumentElement(offsetParent);
  const rect = getBoundingClientRect(element2, true, strategy === "fixed", offsetParent);
  let scroll = {
    scrollLeft: 0,
    scrollTop: 0
  };
